---
title: チャネルオーサリングでデータ統合を有効にする
description: Adobe Journey Optimizer に外部データを統合して、アウトバウンドチャネルをまたいでパーソナライゼーションを強化する方法について説明します。
feature: Personalization, API, Channel Configuration
role: Admin, User
level: Intermediate
doc-type: Feature Video
duration: 212
last-substantial-update: 2026-04-28T00:00:00Z
jira: KT-20916
exl-id: a056114d-24d4-4168-a9c8-6219570a5261
source-git-commit: 9278ff8df1e661aadb54ae0abd4de9e4fed9288d
workflow-type: ht
source-wordcount: '81'
ht-degree: 100%

---

# チャネルオーサリングで外部データ統合を有効にする

Adobe Journey Optimizer に外部データとコンテンツを統合する方法について説明します。これは、アウトバウンドチャネルでのパーソナライゼーションを強化し、エンゲージメントを向上させるのに役立ちます。詳しくは、以下のビデオをご覧ください。

>[!VIDEO](https://video.tv.adobe.com/v/3484118/?learn=on&enablevpops)

詳しくは、Adobe Journey Optimizer [製品ドキュメント](https://experienceleague.adobe.com/ja/docs/journey-optimizer/using/content-management/combine/vendor-integration)を参照してください。
