---
title: オファー、選択戦略、ランキング式の作成
description: オファーの作成方法、オファーを選択するための選択戦略、オファーの優先順位を決定するためのランキング式について説明します
feature: Decisioning
role: User
level: Beginner
doc-type: Tutorial
last-substantial-update: 2025-10-31
jira: KT-19558
exl-id: 03c8c6a7-399b-4d4d-aff1-0500a2de356e
source-git-commit: d4cc60f4448caec92f704026783e2bbe029427f5
workflow-type: tm+mt
source-wordcount: '52'
ht-degree: 0%
---
# オファー、選択戦略、ランキング式の作成

このビデオでは、オファーの作成方法、オファーを選択するための選択戦略、オファーを優先するためのランキング式を説明します
>[!VIDEO](https://video.tv.adobe.com/v/3476427/?learn=on)
