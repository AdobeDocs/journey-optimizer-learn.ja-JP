---
title: Adobe Journey Optimizerでのweb プッシュ
description: Adobe Journey Optimizerでweb プッシュメッセージを送信する方法について説明します
user-guide-title: Adobe Journey Optimizerのキャンペーンとジャーニーを使用してプッシュメッセージを送信する
user-guide-description: このチュートリアルでは、Adobe Journey Optimizerでweb プッシュメッセージを送信するために必要な手順を、キャンペーンとジャーニーの
solution: Journey Optimizer
role: Developer
level: Beginner
breadcrumb-url: /docs/journey-optimizer-learn/ajo-web-push/introduction
source-git-commit: cd96c5cce9682807d453159898ad94e9ca7575b8
workflow-type: tm+mt
source-wordcount: '76'
ht-degree: 7%

---


# Adobe Journey Optimizerでのweb プッシュ{#ajo-web-push}

+ [はじめに](./introduction.md)
+ [ローカル設定](./run-application-locally.md)
+ [プッシュ設定の作成](./create-push-configuration.md)
+ [データストリームを作成](./create-data-stream.md)
+ [ユーザー権限の取得](./capture-user-persmission.md)
+ [オーディエンスを作成](./create-audience.md)
+ [キャンペーンを作成](./create-campaign.md)
+ [タグのプロパティを作成](./create-tag-property.md)
+ [ジャーニーを作成](./create-journey.md)
+ [デバッグ](./debugging.md)

