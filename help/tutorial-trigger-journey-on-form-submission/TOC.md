---
user-guide-title: フォーム送信時のトリガージャーニー
user-guide-description: フォーム送信時のメールアクションを使用したジャーニーのトリガーを学ぶ
role: Developer
level: Beginner
breadcrumb-title: トリガージャーニーのフォーム送信時
breadcrumb-url: /docs/journey-optimizer-learn/trigger-journey-on-form-submission/introduction
auto-video-transcripts: true
source-git-commit: d742353780dbbdbf2dede31b50a779399a7de75d
workflow-type: tm+mt
source-wordcount: '36'
ht-degree: 2%

---


# フォーム送信時のトリガージャーニー{#trigger-journey-on-form-submission}

+ [はじめに](./introduction.md)
+ [カスタムイベントを作成](./custom-event.md)
+ [HTTP ストリーミングを作成](./http-streaming.md)
+ [ジャーニーを作成](./create-journey.md)
+ [ソリューションのテスト](./test-solution.md)
+ [ルックアップデータセットの作成](./lookup-dataset.md)


