---
title: カスタムアクションを使用した、チャレンジ属性によるコンテンツのパーソナライズ
description: カスタムアクションを使用してチャレンジデータを取得し、各メンバーのチャレンジの進捗状況と報酬に基づいて、コンテンツカードとコードベースのエクスペリエンスを動的にパーソナライズする方法を説明します。
role: User
level: Experienced
doc-type: Feature Video
duration: 475
last-substantial-update: '2026-09-11T00:00:00.000Z'
jira: KT-22597
autotag-review: '2026-09-11T19:33:33.401Z'
TQID: 'https://experienceleague.adobe.com/p-OpyR7j7OiMGJbQCLITCniQaciNN3-Brby855pxVK8'
product_v2:
  - id: cb954087-f4fc-4456-afb9-e939cabcdc79
feature_v2:
  - id: 1210f62b-45d9-5d36-8d8d-0f9049ffa368
  - id: 2cb9b022-32b0-573b-a511-d091a4f355fb
  - id: df64005d-8f9a-422e-ba4d-c6f6dc3454b4
subfeature_v2:
  - id: d48edf2f-7bae-4df0-a9d4-7cabfb867d23
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: d378ca77-2da1-4f39-ad92-1917fe974a38
topic_v2:
  - id: e0eb8757-182f-49f3-94a4-1587d16f5094
source-git-commit: b812dd2c5228314771c572a184ae396aa400d524
workflow-type: tm+mt
source-wordcount: 76
ht-degree: 0%

---


# カスタムアクションを使用した、チャレンジ属性によるコンテンツのパーソナライズ

カスタムアクションを使用してチャレンジデータを取得し、各メンバーのチャレンジの進捗状況と報酬に基づいて、コンテンツカードとコードベースのエクスペリエンスを動的にパーソナライズする方法を説明します。

>[!VIDEO](https://video.tv.adobe.com/v/3503375/?learn=on&enablevpops)
