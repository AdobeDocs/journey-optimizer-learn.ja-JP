---
title: クリエイティブツールをまたいでブランドガイドラインを拡張
description: Adobe Journey Optimizerで設定されたブランドを、GenStudioやAdobe ExpressなどのAdobeツールに適用する方法を説明します。
feature: Content Assistant
topic: Content Management
role: User
level: Intermediate
doc-type: Feature Video
duration: 216
last-substantial-update: 2026-07-15T00:00:00Z
jira: KT-20382
source-git-commit: e998ff7cc7d2e977c507974e2c765507e7f1d788
workflow-type: tm+mt
source-wordcount: '96'
ht-degree: 0%

---


# クリエイティブツールをまたいでブランドガイドラインを拡張

このステップでは、Adobe Journey Optimizerで設定したブランドを、GenStudioやAdobe ExpressなどのAdobeツールに適用する方法を説明します。 Adobe Journey Optimizerを使用して、書き方の設定や視覚的な仕様を微調整したり、テキストや画像の例を直接追加したりすることで、AIが生成したコンテンツや自動品質チェックを改善できます。 また、複数のブランドを管理して、さまざまな部門やコミュニケーション戦略をサポートすることもできます。

>[!VIDEO](https://video.tv.adobe.com/v/3494283/?captions=jpn&learn=on)
