---
user-guide-title: Journey Optimizerでのブランドに即したコンテンツの制作
user-guide-description: このチュートリアルでは、ブランドガイドラインの作成と設定から、完全なキャンペーンの生成、個々のコンテンツコンポーネントの編集、ビジュアルの強化、多言語体験の実現に至るまで、あらゆる作業を順を追って説明します。 また、自動化された品質チェックを適用し、ブランドの整合性を確保し、あらゆるコミュニケーションでガバナンスを維持する方法についても解説します。 最終的には、コンテンツ制作を合理化し、一貫性を向上させ、チャネル全体でパーソナライズされた高品質な体験を提供できるようになります。
role: User
level: Internediate
breadcrumb-title: create-on-brand-content
breadcrumb-url: /docs/journey-optimizer-learn/reate-on-brand-content/introduction
auto-video-transcripts: true
source-git-commit: 8a5f2a79a6f7ad8e0d42d5d148ea5a2434e67b26
workflow-type: tm+mt
source-wordcount: '122'
ht-degree: 2%

---


# Journey Optimizerでのブランドに即したコンテンツの制作 {#create-on-brand-content}

+ 概要とブランド設定 {#introduction-and-brand-setup}
   + [概要](./introduction.md)
   + [ブランドの設定](./configure-brands.md)
   + [ブランドガイドラインのヒント](./brands-tips-and-tricks.md)
+ コンテンツの生成と調整 {#generate-content}
   + [ゼロからコンテンツを制作](./create-content-from-scratch.md)
   + [セクションコンテンツの編集とパーソナライズ](./edit-and-personalize-section-content.md)
   + [ビジュアルコンテンツを強化](./enhance-your-visual-content.md)
   + [多言語バージョンのコンテンツを作成および調整](./create-and-refine-multilingual-content.md)
+ [品質、ブランドの整合性、ガバナンスを確保](./quality-brand-alignment-and-governance.md)



