...
**コンテンツカード** は、アプリ内の専用のインボックスまたはフィード内に表示される、永続的なアプリ内メッセージメッセージです。 プッシュ通知とは異なり、プッシュ通知はユーザーを中断せず、ユーザーが選択したときにいつでも表示できます。

これらの製品は、より長い可視性と繰り返しの露出の恩恵を受ける **緊急ではない**、**情報**、または **プロモーションコンテンツ** に最適です。 コンテンツカードは、ユーザーが自分のペースで更新を閲覧できる、アプリ内の「メッセージセンター」またはパーソナライズされたコンテンツフィードと考えてください。」
